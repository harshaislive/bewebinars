...
Provision `CALENDLY_CLIENT_ID`, `CALENDLY_CLIENT_SECRET`, `CALENDLY_REDIRECT_URI`, `ADMIN_USER`, `ADMIN_PASS`, and `SESSION_SECRET` in `.env`, and never commit that file or `tokens.json`. For Zoom attendance analytics, also set `ZOOM_ACCOUNT_ID`, `ZOOM_CLIENT_ID`, and `ZOOM_CLIENT_SECRET` (server-to-server OAuth). Rotate Calendly or Zoom credentials whenever access is revoked, and clear stale session cookies when testing login flows in multiple browsers.
